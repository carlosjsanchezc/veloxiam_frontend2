...
export class ContactsService
{
    ...
    {
        return this._httpClient.get<Contact[]>(environment._apiUrl+'contacts').pipe(
            tap((contacts) => {
                console.log("Los cr",contacts);
                contacts.forEach(element => {
                   // element.emails=JSON.parse(element.emails);
                   element.tags=[];
                });
    ...
    createContact(): Observable<Contact>
    {
        return this.contacts$.pipe(
            take(1),
            switchMap((contacts) => this._httpClient.post<Contact>(environment._apiUrl+'contact/new', {}).pipe(
                map((newContact) => {
    ...
    updateContact(id: string, contact)
    {
        ...
        return this._httpClient.post(environment._apiUrl+"contacts/"+id,contact).toPromise();
    ...
    {
        return this._httpClient.post(environment._apiUrl+"contact/delete/"+id,{}).toPromise();
